...
public class GameBoard {
    ...
    private void openAndPushCellAt(Stack<CellPosition> stack) {
        ...
        if (isLandMineCellAt(cuurentCellPosition)) {
            return;
        }

        openOneCellAt(cuurentCellPosition);

        if (doesCellHaveLandMineCount(cuurentCellPosition)) {
            return;
        }

        List<CellPosition> surroundedPositions = calculateSurroundedPositions(cuurentCellPosition, getRowSize(), getColSize());
        for (CellPosition surroundedPostion : surroundedPositions) {
            stack.push(surroundedPostion);
        }
    }
